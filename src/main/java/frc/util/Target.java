...
import frc.utilPackage.LowPassFilter;
...
public class Target{
    ...
    public void display() {
        double[] cont1 = {ct1.getCenter().getX(), ct1.getCenter().getY()};
        double[] cont2 = {ct2.getCenter().getY(), ct2.getCenter().getY()};
        double distance = filter.update(calcDistance());
        SmartDashboard.putNumber("Distance", distance / Units.Length.inches);
        double contourDistance = Math.pow(Math.pow(cont1[0] - cont2[0], 2) + Math.pow(cont1[1] - cont2[1], 2), 0.5);
        SmartDashboard.putNumber("Distance Between Contours", contourDistance);
        SmartDashboard.putNumber("Angle I guess", calculateAngle(contourDistance));
        SmartDashboard.putNumberArray("Contour 1", cont1);
        SmartDashboard.putNumberArray("Contour 2", cont2);
    }
    public double calculateAngle(double distBetweenContours){
//        return -53.99074 - (-0.0002062282/-0.0327877)*(1 - Math.exp(+0.0327877*distBetweenContours));
        double x = distBetweenContours;
        double y = -282.4246 + 3.106912*x - 0.007375874*x*x;
        return y;
    }
}
